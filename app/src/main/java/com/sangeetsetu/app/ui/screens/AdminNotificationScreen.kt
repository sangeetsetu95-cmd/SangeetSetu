...
@Composable
fun AdminNotificationScreen(onBack: () -> Unit) {
    PremiumAdminTheme {
        Scaffold(
            topBar = {
                TopAppBar(
                    title = { Text("Admin Alerts", color = PremiumGold, fontWeight = FontWeight.ExtraBold) },
                    navigationIcon = {
            ...
            Box(modifier = Modifier.padding(padding).fillMaxSize(), contentAlignment = Alignment.Center) {
                Text("No system alerts at this time", color = AdminSecondaryText)
            }
        }
    }
}
